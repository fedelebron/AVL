...
insertUnbalancedAt t (LRC v y ctx) = zipUp $ Zipper (Balanced v y t) ctx
insertUnbalancedAt t (RLC v y ctx) = zipUp $ Zipper (Balanced v t y) ctx
insertUnbalancedAt t Root = T t

{- LLC -}
insertUnbalancedAt (Leftie b g p) (LLC a d ctx) = zipUp z
  where
    z = Zipper (Balanced b g (Balanced a p d)) ctx
insertUnbalancedAt (Rightie b g q) (LLC a d ctx) = zipUp z
  where
    z = Zipper t ctx
    t = case q of
...
{- RRC -}
insertUnbalancedAt (Rightie b g p) (RRC a d ctx) = zipUp z
  where
    z = Zipper (Balanced b (Balanced a d g) p) ctx
insertUnbalancedAt (Leftie b q p) (RRC a d ctx) = zipUp z
  where
    z = Zipper t ctx
    t = case q of
...
rebalance t (RLC a (Balanced d t1 t2) ctx) = zipUp z
  where
...
rebalance t (LRC a (Balanced d t1 t2) ctx) = zipUp $ z
  where
